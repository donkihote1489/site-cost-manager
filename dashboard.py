...
def summary_dashboard():
    ...
    with st.expander("📌 전체 테이블 보기", expanded=True):
        df_fmt = df.copy()
    ...
    with st.container():
        ...
        if st.button("선택한 데이터 삭제", use_container_width=True):
    ...
    with st.expander("📈 요약 수치 및 그래프", expanded=True):
    ...
    ax1.legend(title="비용항목", fontsize=9, title_fontsize=10)
    ...
    _, unit_label2, unit_div2 = format_unit(df_cumsum[["기성금", "투입비", "노무비"]].values.max())
